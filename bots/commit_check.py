...
Usage:
    graphify bot commit-check [<ref>] [--repo PATH]
...
CRITERIA = """\
Review this single commit's diff. Use the graphify MCP tools to check
...
def main() -> int:
    ...
    try:
        ...
        with common.Task("commit-check", repo, args.graphify_bin, need, args.skip_preflight) as t:
            prompt = f"{CRITERIA}\n\nCommit {args.ref}:\n```diff\n{diff}\n```"
