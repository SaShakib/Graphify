...
Usage:
    graphify bot triage <issue-number> [--repo PATH] [--comment]
"""
...
PROMPT = """\
Triage this GitHub issue against the codebase. Use graphify's MCP tools:
...
def main() -> int:
    ...
    try:
        ...
        need = {"gh authenticated", "claude session connects", "graphify MCP server"}

        meta_raw = common.gh(["issue", "view", str(args.issue), "--json", "title,body"], cwd=repo)
        meta = json.loads(meta_raw)

        with common.Task("triage", repo, args.graphify_bin, need, args.skip_preflight) as t:
...
def render(number: int, b: dict) -> str:
    lines = [f"## 🤖 graphify triage — issue #{number}", "", b.get("summary", ""), ""]
    ...
    if rel:
        ...
        for r in rel:
            lines.append(f"- {r}")
    lines.append("")
    lines.append("_Correlated using the graphify code graph + vector memory._")
    return "\n".join(lines)
