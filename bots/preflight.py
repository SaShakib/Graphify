#!/usr/bin/env python3
"""Preflight checks shared by every graphify bot.
...
directly (`python3 bots/preflight.py`, or `graphify bot doctor`).
"""
...
def check_claude_mcp(graphify_bin: str | None, repo: Path) -> Check:
    ...
    independently via the graphify binary."""
    if not graphify_bin:
        return Check("graphify MCP server", False, "no graphify binary located", "build with `go build -o graphify ./cmd/graphify`")
    ...
    try:
        ...
        return Check("graphify MCP server", False, "timed out", "check `graphify mcp .` runs")
    ...
    return Check(
        ...
        "" if ok else "check `graphify mcp .` and that the repo is indexed",
    )


def find_graphify_bin(explicit: str | None = None) -> str | None:
    if explicit:
        return explicit
...
def run_checks(repo: Path, graphify_bin: str | None = None, include_mcp: bool = True) -> list[Check]:
    bin_path = find_graphify_bin(graphify_bin)
    checks = [
        check_python(),
        check_gh_installed(),
        check_gh_auth(),
...
def main() -> int:
    ...
    parser.add_argument("--graphify-bin", default=None)
    args = parser.parse_args()

    repo = Path(args.repo).resolve()
    checks = run_checks(repo, args.graphify_bin)

    print("graphify orchestration preflight\n")
    all_ok = True
    for c in checks:
        mark = "✓" if c.ok else "✗"
