...
// EmbedderFromEnv returns an HTTPEmbedder when GRAPHIFY_EMBED_URL is set,
...
//
//	GRAPHIFY_EMBED_URL   e.g. http://localhost:11434/v1  (required to enable)
//	GRAPHIFY_EMBED_MODEL e.g. nomic-embed-text           (required to enable)
//	GRAPHIFY_EMBED_KEY   API key, if the endpoint needs one (optional)
//	GRAPHIFY_EMBED_DIM   model output dimension           (default 768)
//
...
func EmbedderFromEnv() Embedder {
	url := os.Getenv("GRAPHIFY_EMBED_URL")
	model := os.Getenv("GRAPHIFY_EMBED_MODEL")
	if url == "" || model == "" {
		return NewHashingEmbedder(512)
	}
	dim := 768
	if d := os.Getenv("GRAPHIFY_EMBED_DIM"); d != "" {
		if n, err := parseDim(d); err == nil {
			dim = n
		}
	}
	return NewHTTPEmbedder(url, model, os.Getenv("GRAPHIFY_EMBED_KEY"), dim)
}
