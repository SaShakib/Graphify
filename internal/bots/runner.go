...
// Runner executes bots by re-invoking the graphify binary and tracks their
// runs in memory. Safe for concurrent use. Runs are not persisted — this is
// a local dev tool; restarting graphify clears run history.
type Runner struct {
	selfBin  string // path to the graphify binary to re-invoke
	repoRoot string
...
// NewRunner builds a Runner. selfBin is the graphify executable to invoke
