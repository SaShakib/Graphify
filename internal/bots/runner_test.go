...
// controllable code, standing in for the real graphify binary so the runner
// is tested hermetically (no graphify/claude needed).
func fakeBin(t *testing.T, exitCode int) string {
	...
	dir := t.TempDir()
	path := filepath.Join(dir, "fake-graphify")
	script := "#!/bin/sh\necho \"args: $@\"\nexit " + itoa(exitCode) + "\n"
