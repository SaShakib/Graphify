...
func memoryDBPath(root string) string {
	return filepath.Join(root, ".graphify", "memory.db")
}

func openMemory(repoRoot string) (*memory.Store, error) {
	if err := os.MkdirAll(filepath.Join(repoRoot, ".graphify"), 0o755); err != nil {
		return nil, err
	}
	// EmbedderFromEnv picks a neural embedder if GRAPHIFY_EMBED_URL is set,
...
func newMemoryCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `graphify's vector memory holds non-code knowledge — primary codebase
...
<repo>/.graphify/memory.db, separate from the graph so re-indexing never
wipes it.`,
	}
...
func newMemoryListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(entries) == 0 {
				cmd.Println("No memory entries yet. Add one with `graphify memory add`.")
				return nil
			}
